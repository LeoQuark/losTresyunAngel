<!doctype html>
<html lang="en">

<head>
    <!-- Required meta tags -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <script src="https://kit.fontawesome.com/b097035380.js" crossorigin="anonymous"></script>
    <!-- Bootstrap CSS -->
    <link rel="stylesheet" href="../../css/bootstrap.min.css">
    <link rel="stylesheet" href="../../css/estilos.css">
    <link rel="stylesheet" href="../../css/secciones.css">
    <!-- CHARTS GRAPHICS -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/2.9.3/Chart.min.js"></script>
    <link rel="icon" href="../../img/icono.png">
    <title>Tres y un Ángel</title>
</head>

<body class="body-portada">
    <!-- navbar -->
    <nav class="navbar navbar-expand-lg fixed-top fondo-nav">
        <div class="container">
            <div class="row">
                <a href="#"><img src="../../img/icono.png" class="img-icono" alt="descripcion" width="35"
                        height="40"></a>
                <a href="#" class="navbar-brand mx-3 pt-2 font-lostres titulo-portada">los tres y un
                    ángel</a>
            </div>
            <button class="navbar-toggler border border-light shadow-sm" type="button" data-toggle="collapse"
                data-target="#navbar" aria-controls="navbar" aria-expanded="false" aria-label="Menu">
                <span class="material-icons text-white"><em class="fas fa-bars text-white"></em></span>
            </button>
            <div class="collapse navbar-collapse justify-content-end" id="navbar">
                <div class="row">
                    <ul class="navbar-nav pr-3">
                        <li class="nav-item">
                            <a href="../dynamics/inicio.php" class="nav-link mx-2 font-nav">Inicio</a>
                        </li>
                        <li class="nav-item">
                            <a href="../dynamics/ventas.php" class="nav-link mx-2 font-nav">Ventas</a>
                        </li>
                        <li class="nav-item">
                            <a href="../dynamics/productos.php" class="nav-link mx-2 font-nav">Productos</a>
                        </li>
                        <li class="nav-item">
                            <a href="../dynamics/proveedor.php" class="nav-link mx-2 font-nav">Proveedor</a>
                        </li>
                        <li class="nav-item">
                            <a href="../dynamics/bodega.php" class="nav-link mx-2 font-nav">Bodega</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </nav><br>
    <br>
    <br>